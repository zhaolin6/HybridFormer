...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG, patch_size = 11):
    ...
    if dataset_name == 'PaviaC':
    ...
    elif dataset_name == 'HyRANK_satellite':
        folder = os.path.join(folder, "TrainingSet")
        img = open_file(os.path.join(folder, "Dioni.tif"))
        # img = gdal.Open(folder + "Dioni.tif")
        # img = img.ReadAsArray()
        rgb_bands = (55, 41, 12)
        gt = open_file(os.path.join(folder, "Dioni_GT.tif"))
        num_cls = np.unique(gt)
        # gt = gt.ReadAsArray()
        label_values = ["Undefined", "Dense Urban Fabric", "Mineral Extraction Sites",
                        "Non Irrigated Arable Land",
                        "Fruit Trees", "Olive Groves", "Broad-leaved Forest",
                        "Coniferous Forest", "Mixed Forest", "Dense Sclerophyllous Vegetation",
                        "Sparce Sclerophyllous Vegetation", "Sparcely Vegetated Areas",
                        "Water", "Coastal Water", "Coastal Water"]
        ignored_labels = []
        ...
        ppp = 'E:\lb\Hyperspectral-Classification-master\Datasets'
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.radiation_augmentation and np.random.random() < 0.2:

...
from compara_emperiment import *

from HybirdSN import HybirdSN
from vit_SS import ViT


def get_model(name, ps=[3, 5], d_h=[4,2], **kwargs):
    """
    ...
    """
    device = kwargs.setdefault('device', torch.device('cuda:0'))  # default:cpu
    ...
    if name == 'hybridformer':  # patch size 代表模型输入的大小，影响计算量
        patch_size = kwargs.setdefault('patch_size', 15)  # 19\15\11
        center_pixel = True
        model = ViT(image_size=patch_size, patch_size=ps, num_classes=n_classes, dim=100, depth=d_h[0], heads=d_h[1],
                    mlp_dim=2048, channels=n_bands, dropout=0.2, emb_dropout=0.2)
        lr = kwargs.setdefault('learning_rate', 0.0001)  # 0.0001
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        kwargs.setdefault('epoch', 200)  # default 100
        kwargs.setdefault('batch_size', 128)
    elif name == 'FDSSC':
        patch_size = kwargs.setdefault('patch_size', 15)  # 19\15\11
        center_pixel = True
        model = FDSSC_network(n_bands, n_classes)
        lr = kwargs.setdefault('learning_rate', 0.0001)  # 0.0001
        optimizer = optim.Adam(model.parameters(), lr=lr)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
        kwargs.setdefault('epoch', 200)  # default 100
        ...
        model = DBDA_network_MISH(band=n_bands, classes=n_classes)
        lr = kwargs.setdefault('learning_rate', 0.0005)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault('batch_size', 100)  # 30 samples=128,10 samples=64
        kwargs.setdefault('epoch', 200)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    elif name == 'SSRN':
        patch_size = kwargs.setdefault('patch_size', 15)  # default = 21
        center_pixel = True
        model = SSRN_network(band=n_bands, classes=n_classes)
        lr = kwargs.setdefault('learning_rate', 0.0005)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault('batch_size', 128)  # 30 samples=128,10samples=64
        kwargs.setdefault('epoch', 200)
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    elif name == 'nn':
        ...
        center_pixel = True
        self_attention = False
        model = HybirdSN(num_classes=n_classes, channels=n_bands,  self_attention=self_attention)
        ...
        patch_size = kwargs.setdefault('patch_size', 11)  # default = 11
        center_pixel = True
        model = Cnn2D(n_bands, n_classes, patch_size=patch_size)
        lr = kwargs.setdefault('learning_rate', 0.0001)  # 设置的0.001
        # optimizer = optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
        optimizer = optim.Adam(model.parameters(), lr=lr)
        kwargs.setdefault('batch_size', 128)  # 30 samples=128,10 samples=64
        kwargs.setdefault('epoch', 200)  # default 100
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
    ...
    kwargs.setdefault('batch_size', 64)  # 10samples 64//30samples 128
...
def train(net, optimizer, criterion, data_loader, epoch, scheduler=None,
          ...
          val_loader=None, supervision='full', args=None):
    """
    ...
    file = open("FLOPS.txt", 'a+')
    file.write(args.MODEL)
    file.write("\n")
    file.write(args.DATASET)
    file.write("\n")
    file.write("Trainning Time:")
    file.write(str(time_use))
    file.write("\n")
    file.close()
...
def test(net, img, hyperparams, args=None):
    """
    ...
    for batch in tqdm(grouper(batch_size, sliding_window(img, **kwargs)),
        ...
        with torch.no_grad():
            ...
            start = time.time()
            output = net(data)
            time_use = time_use + time.time() - start
...
def test4loader(net, test_loader, device, args=None):
    """
    Test a model on a specific image
    """
    net.eval()
    start = time.time()
    for data, label in tqdm(test_loader, total=len(test_loader), desc="Inference on the image"):
        data = data.to(device)
        net(data)
    time_use = time.time() - start
    file = open("FLOPS.txt", 'a+')

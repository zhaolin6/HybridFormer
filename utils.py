...
def metrics(prediction, target, ignored_labels=[], n_classes=None):
    ...
    """
    ignored_mask = np.zeros(target.shape[:2], dtype=np.bool) # target 维度145
...
def show_results(results, vis, label_values=None, agregated=False):
    ...
    if agregated:
        ...
        for label, score in zip(label_values, F1scores):
            # t += "\t{}: {:.03f}\n".format(label, score)
            text += "\t{}: {:.03f}\n".format(label, score)
        # f = open('comparison experiments', 'a+')
        # f.write(t)
        # f.close()
    text += "---\n"
...
def sample_gt(gt, train_size, ignore=[], mode='random'):
    ...
    if mode == 'random':
       ...
       for c in np.unique(gt):
           ...
           # train, test = sklearn.model_selection.train_test_split(X, train_size=0.1)
           train_indices += train
           test_indices += test
